...
def predict(Xtr, Ytr, k, v, metric=None):
    ...
    Yts = np.zeros((int(0.2*Xtr.shape[0]), 1))
    for i in range(v, v + int(0.2*Xtr.shape[0])-1):
        ...
        for x in range(v + int(0.2*Xtr.shape[0]),Xtr.shape[0]):
            neighbors.append([int(Ytr[x]),dist[x]])
        ...
        correct = 0
        for x in range(v, v + int(0.2*Xtr.shape[0])-1):
            if Ytr[x] == Yts[x]:
                correct += 1
        acc = (correct/float(len(Yts))) * 100.0
        '''
...
def main(): 
    ...
    for v in range(0,4):
        for k in range(1,100):
            acc = predict(Xtr, Ytr, k, v*int(0.2*Xtr.shape[0]), metric)
            f = open('myfile', 'a')
